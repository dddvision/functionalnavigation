classdef ReferenceTrajectory < tom.Trajectory & TrajectorySim.TrajectorySimConfig
  ...
  methods (Access = public)
    function this = ReferenceTrajectory(initialTime)
      % read the tangent pose file
      filename = fullfile(fileparts(mfilename('fullpath')), this.tangentPoseFileName);
      [this.tRef, this.pRef, this.qRef, this.rRef, this.sRef] = readTangentPoseFile(filename);
      K = numel(this.tRef);

      % add initial time
      this.tRef = this.tRef+initialTime;
      
      % ensure that quaternions are unit magnitude to machine precision
      this.qRef = QuatNorm(this.qRef);

      % preprocessing incremental rotations and ensuring positive first element
      this.dqRef = zeros(4, K-1);
      for j = 1:(K-1)
        this.dqRef(:, j) = Quat2Homo(QuatConj(this.qRef(:, j)))*this.qRef(:, j+1); 
        this.dqRef(:, j) = this.dqRef(:, j)*sign(this.dqRef(1, j));
      end
...
function [dt, p, q, r, s] = readTangentPoseFile(filename)
